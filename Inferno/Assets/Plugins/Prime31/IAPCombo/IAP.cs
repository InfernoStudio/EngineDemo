...
namespace Prime31
{
	public static class IAP
	{
        ...
        private static Action<bool, GooglePurchase> _verifyAction;
        ...
        // Purchases the given product and quantity. completionHandler provides if the purchase succeeded (bool) and an error message which will be populated if
        // the purchase failed.
        public static void purchaseConsumableProduct(string productId, Action<bool, string> completionHandler,Action<bool,GooglePurchase> verifyHandler = null)
        {
            _purchaseCompletionAction = completionHandler;
            _purchaseRestorationAction = null;

#if UNITY_ANDROID
            _verifyAction = verifyHandler;
            GoogleIAB.purchaseProduct(productId, CONSUMABLE_PAYLOAD);
#elif UNITY_IOS || UNITY_TVOS
			StoreKitBinding.purchaseProduct( productId, 1 );
#endif
        ...
        // Purchases the given product and quantity. completionHandler provides if the purchase succeeded (bool) and an error message which will be populated if
        // the purchase failed.
        public static void purchaseNonconsumableProduct(string productId, Action<bool, string> completionHandler, Action<bool, GooglePurchase> verifyHandler = null)
        {
            _purchaseCompletionAction = completionHandler;
            _purchaseRestorationAction = null;
